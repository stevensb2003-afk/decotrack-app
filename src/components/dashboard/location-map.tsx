...
function PlacesAutocomplete({ onSelect, initialAddress }: { onSelect: (details: {latitude: number, longitude: number, address: string} | null) => void, initialAddress?: string }) {
    ...
    useEffect(() => {
        ...
        const listener = autocomplete.addListener('place_changed', () => {
            ...
            if (place.geometry?.location) {
                ...
                onSelect({ latitude: position.lat, longitude: position.lng, address: place.formatted_address || '' });
            } else {
                onSelect(null);
            }
        });
    ...
    return (
        <div>
            ...
            <Input ref={inputRef} id="location-search" placeholder="e.g., 123 Main St, Anytown" defaultValue={initialAddress || ''} />
        </div>
    )
}
      ...
      <PlacesAutocomplete 
        ...
        onSelect={(details) => {
            if (details) {
                ...
                map?.moveCamera({ center: {lat: details.latitude, lng: details.longitude}, zoom: 15 });
                onLocationChange({
