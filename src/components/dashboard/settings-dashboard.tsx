...
import { MoreHorizontal, UserPlus, Eye, Trash2, KeyRound } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function SettingsDashboard() {
  ...
  const [userToChangePassword, setUserToChangePassword] = useState<SystemUser | null>(null);
  ...
  const [nextAction, setNextAction] = useState<'viewPassword' | 'changePassword' | null>(null);
  ...
  const handleViewPasswordClick = (user: SystemUser) => {
    ...
    setNextAction('viewPassword');
    setIsCodePromptOpen(true);
    setSecurityCodeInput("");
  };

  const handleChangePasswordClick = (user: SystemUser) => {
    setUserToChangePassword(user);
    setNextAction('changePassword');
    setIsCodePromptOpen(true);
    setSecurityCodeInput("");
  };
  ...
  const handleSecurityCodeSubmit = () => {
    if (securityCodeInput === SECURITY_CODE) {
      ...
      if (nextAction === 'viewPassword') {
          setIsPasswordViewOpen(true);
      } else if (nextAction === 'changePassword') {
          setNewPassword('');
          setConfirmPassword('');
  ...
  const handleConfirmPasswordChange = async () => {
      ...
      if (!newPassword || newPassword !== confirmPassword) {
          toast({ title: "Passwords do not match or are empty.", variant: "destructive"});
          return;
      }
      ...
      await updateUserPassword(userToChangePassword.id, newPassword);
      toast({ title: "Password Updated", description: `Password for ${userToChangePassword.email} has been changed.`});
  ...
  return (
    <>
            ...
            <TableBody>
              {users.map((user) => (
                <TableRow key={user.id}>
                  <TableCell className="font-medium">{employeeNameMap.get(user.email) || `${user.firstName} ${user.lastName}`.trim() || 'Admin'}</TableCell>
      ...
      <Dialog open={isCodePromptOpen} onOpenChange={setIsCodePromptOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              To perform this action, please enter the administrator security code.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
      ...
      <Dialog open={isPasswordViewOpen} onOpenChange={setIsPasswordViewOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <Alert>
              <AlertTitle>{employeeNameMap.get(selectedUserForPassword?.email || '') || 'Admin'}</AlertTitle>
              <AlertDescription>
      ...
      <Dialog open={isChangePasswordDialogOpen} onOpenChange={setIsChangePasswordDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Change Password for {userToChangePassword?.email}</DialogTitle>
            <DialogDescription>
